...
Then the evaluated expression text should be "4 + 3"
    And the expression result text should be "4 + 3"
    And the expression result value should be "7"
...
Then the evaluated expression text should be "4 - 3"
    And the expression result text should be "4 - 3"
    And the expression result value should be "1"

Scenario: Evaluating dice rolls
Given the home page is open
    And the random number generator name is "constantMax"
When the expression "3d6" is evaluated
Then the evaluated expression text should be "3d6"
    And the expression result text should be "6 [d6] + 6 [d6] + 6 [d6]"
    And the expression result value should be "18"

Scenario: Evaluating dice rolls and constants
Given the home page is open
    And the random number generator name is "constantMax"
When the expression "3d6+4" is evaluated
Then the evaluated expression text should be "3d6 + 4"
    And the expression result text should be "6 [d6] + 6 [d6] + 6 [d6] + 4"
